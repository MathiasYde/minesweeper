...
public class Minesweeper {
    ...
    private Void onClickCell(Int2D position) {
        ...
        while (queue.isEmpty() == false) {
            ...
            if (isPositionInBounds(cellPosition) == false) {
                continue;
            }
            ...
            if (cell == CELL_REVEALED_BOMB) {
                continue;
            }
            if (cell == CELL_UNREVEALED_BOMB) {
                continue;
            }
            if (cell == CELL_FLAG_BOMB || cell == CELL_FLAG_NONE_BOMB) {
                continue;
            } // ignore flags

            // cell must now be unrevealed
    ...
    private int countNeighboringBombs(Int2D position) {
        ...
        for (Int2D neighboringCellPosition : neighboringCellPositions) {
            if (isPositionInBounds(neighboringCellPosition) == false) {
                continue;
            }
